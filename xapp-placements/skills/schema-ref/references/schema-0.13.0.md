...
| `xapp_p_*.ad_chain.enable_live_load` | **NEW 0.16.0.** Bool, default **true** (existing behavior — live-load on buffer miss at show). Set false to skip the show-path OWN_LOAD tier (and the parallel_auction floor-wait sub-auction) — placement then shows only preloaded/reused ads — EXCEPT the lazy carveout (primary entry unit `preload_trigger: lazy` + empty buffer + no pending fill → still live-loads). When gated, `show()` returns false with outcome `LIVE_LOAD_DISABLED`. `load()` is NOT gated. |
