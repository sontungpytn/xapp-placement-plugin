...
- `collapsible_v1` — banner that expands to a full card (uses the `collapse_arrow` block). Renders inline. Use for `ncollap_*` collapsible native placements.
- `fullscreen_hero_v1` — fullscreen modal takeover (launches the modal Activity, honors `skip_delay_sec`). All other templates render inline. Use for `nfull_*` interstitial-style native placements.
...
- `collapse_arrow.targets` (`string[]` subset of `{"media","cta"}`, default `["media"]`) — which sections collapse on arrow tap, `collapsible_v1` template ONLY; invalid tokens dropped, empty → default. (NEW 0.12.6)
