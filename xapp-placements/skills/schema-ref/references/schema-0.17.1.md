...
48. NEW (SDK 0.17.1): `xapp_config.preload.load_timeout_ms` present AND not an integer, or outside [1000, 60000] (SDK `coerceIn(1000, 60000)` + WARN; admin `z.number().int().min(1000).max(60000).default(30000)` rejects on import). ABSENT = OK (default 30000). Applies to the PRELOAD path only.
