---
name: schema-ref
description: Print canonical XAppAdKit (xappsdk) JSONC schema reference for SDK 0.13.0 and named UI presets. Use when user asks about xapp_config / xapp_ad_units / xapp_registry / xapp_p_* shape, native ui_config fields, valid format enums, preload_trigger values, capping defaults, or wants to inspect starter ad-unit preset / native UI presets.
...
1. Read `$CLAUDE_PLUGIN_ROOT/skills/schema-ref/references/schema-0.13.0.md` and/or `$CLAUDE_PLUGIN_ROOT/skills/schema-ref/references/presets.md` based on argument:
...
The schema file mirrors `com.xapp.adkit.config.ConfigRepository` DTOs at git tag v0.13.0. Update this reference when SDK version bumps.
