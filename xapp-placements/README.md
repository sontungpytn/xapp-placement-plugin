...
- **0.10.1** (2026-06-29): Doc-only. SDK `feat/adapter-init-timeout` ([MR !50](https://git.xantus.network/app/sdk/x-app-ad-kit-sdk/-/merge_requests/50)) adds `AppConfig.adapterInitTimeoutMs` (Long, default 0) — caps how long `MobileAds.initialize` is awaited before the first ad request; `>0` requests ads after the timeout while adapters finish in the background. An SDK init-API change in app code, NOT a key in the `xapp_*` RC bundle, so generator / validator / schema reference are unaffected. No SDK schema bump (still 0.13.0).
