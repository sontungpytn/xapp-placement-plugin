...
Fast generator for XAppAdKit (xappsdk) ad placement config JSONC files. Output matches Xantus admin import format + SDK 0.18.0 schema.
...
- `/xapp-placements:schema-ref` — print canonical SDK 0.18.0 schema reference.
...
Pinned to `com.xantus:x-app-ad-kit-sdk:0.18.0`. Bump plugin when SDK schema changes. **From 0.16.7 the plugin version tracks the SDK version 1:1** (earlier plugin versions used an independent scheme — see the pre-0.16.7 entries below).

## Changelog

- **0.18.0** (2026-07-29): Sync to SDK 0.18.0. (1) NEW `xapp_config.preload.load_timeout_ms` (int 1000–60000, default 30000) — the PRELOAD-path coroutine timeout, previously hardcoded at 10000ms in `ChainLoader`. Per-unit `xapp_ad_units[*].load_timeout_ms` still overrides it; the show path is unchanged (placement `timing_config.load_timeout_ms`). (2) `xapp_config.preload.max_concurrent_loads` default **3 → 6**, absorbing the longer permit hold in `PreloadScheduler`; admin now enforces the SDK's [1, 8] range on import. Generator preset emits both new values. Validator gains hard-error rule 48 plus two warnings. Schema reference doc renamed `schema-0.16.7.md` → `schema-0.18.0.md`.
