...
- **NEW 0.16.0**: `ad_chain.enable_live_load` present AND not boolean — admin `z.boolean().default(true)` rejects. ABSENT = OK (default true = existing behavior, live-loads on buffer miss). Opt-out only: when explicitly `false`, a placement with no preloaded/reused ad at show and no lazy carveout will NOT live-load (`show()` → `LIVE_LOAD_DISABLED`) — INFO: confirm that's intended for non-lazy primaries with small/absent buffer.
