...
- **NEW 0.16.0**: `ad_chain.enable_live_load` present AND not boolean — admin `z.boolean().default(false)` rejects. ABSENT = OK (default false). NOTE default false is a behavior change — a placement with no preloaded/reused ad at show and no lazy carveout will NOT live-load; flag placements that likely depend on live-load (non-lazy primary, small/absent buffer) as INFO to set `enable_live_load: true` if intended.
