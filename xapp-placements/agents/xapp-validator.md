...
- **NEW (SDK 0.18.0)**: `preload_trigger_by_segment` value must be EITHER a trigger string (legacy form, = `priority: 0`) OR an object `{trigger, priority}` (`priority` int, default 0). Reject/flag any other value — booleans, numbers, arrays, `null` — as a HARD ERROR; admin's `z.union([preloadTriggerEnum, z.object({trigger: preloadTriggerEnum, priority: z.number().int().default(0)})])` rejects all of them on import. Note the SDK's own behavior is NOT uniform across those types: a boolean or number is NOT dropped (`isJsonPrimitive` matches all three) — it flows through `asString` and lands on unknown-trigger WARN → `INIT_DELAYED`, `priority: 0`, the key survives; only an array or `null` is actually dropped + WARN. `priority` present but non-numeric — SDK coerces to `0` + WARN; admin rejects (schema expects `z.number().int()`). Fractional `priority` (e.g. `1.5`) — SDK silently truncates via `asInt` (`1.5` → `1`), no warn at all; admin's `z.number().int()` hard-rejects it, so an operator can hit a value the two sides disagree on. `priority` missing/null is fine (defaults to `0`, no warn).
- `buffer_size < 1`.
...
- **NEW 0.16.0 (per entry)**: `ad_chain.entries[*].enable_live_load` present AND not boolean — admin `z.boolean().default(true)` rejects. ABSENT = OK (default true = existing behavior, entry live-loads on buffer miss). It is a PER-ENTRY field, not a placement-level `ad_chain.enable_live_load` (flag that shape as a WARN: ignored / likely misplaced — move it onto the intended `entries[]`). Opt-out only: an entry with `enable_live_load: false` is excluded from show-time live-load unless the lazy carveout applies; when ALL entries are gated off a buffer-miss show → `LIVE_LOAD_DISABLED` — INFO: confirm intended for non-lazy entries with small/absent buffer.
...
- **NEW (SDK 0.18.0)**: two `preload_trigger_by_segment` keys on the same ad unit could both match one user (a user now holds a SET of segment tags, not one) AND carry the SAME `priority` with DIFFERENT `trigger` values (e.g. `ua_traffic` + `ua_traffic_google` both at the default `priority: 0` — these two DO co-occur, a paid install always carries both) — the SDK breaks the tie on the alphabetically first key, which is rarely what the operator meant. Recommend setting distinct `priority` values on the overlapping segment rows. Do NOT flag a tie between `new_user` and `return_user` specifically — they are mutually-exclusive lifecycle tags (`KnownSegments`, `AppConfig.userSegment`), a user holds exactly one at a time, so those two keys can never both apply and a tie between them is not a real risk.
