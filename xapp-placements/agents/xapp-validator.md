...
- **NEW (SDK 0.18.0)**: `preload_trigger_by_segment` value must be EITHER a trigger string (legacy form, = `priority: 0`) OR an object `{trigger, priority}` (`priority` int, default 0). Reject/flag non-string, non-object values (e.g. arrays) — SDK drops them with WARN; admin `z.union([preloadTriggerEnum, z.object({trigger: preloadTriggerEnum, priority: z.number().int().default(0)})])` rejects on import. `priority` present but non-numeric — SDK coerces to `0` + WARN; admin rejects (schema expects `z.number().int()`). `priority` missing/null is fine (defaults to `0`, no warn).
- `buffer_size < 1`.
