...
unzip xapp-placements-v0.6.0.zip -d ~/xantus-plugins
...
```bash
unzip xapp-placements-v0.6.0.zip -d /tmp/xappsdk
mkdir -p ~/.claude/plugins/cache/xappsdk/xapp-placements/0.6.0
cp -R /tmp/xappsdk/xappsdk/xapp-placements/* ~/.claude/plugins/cache/xappsdk/xapp-placements/0.6.0/
...
- `/xapp-placements:validate` — validate file against SDK 0.11.9 schema
...
SDK pinned: `com.xantus:x-app-ad-kit-sdk:0.11.9`.
